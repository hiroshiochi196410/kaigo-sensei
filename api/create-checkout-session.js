export default async function handler(req, res) {
  try {
    ...
    if (!priceId) return res.status(500).json({ error: `Missing STRIPE_PRICE_ID_${variant.toUpperCase()}` });
    ...
    // 決済後：同じ画面に戻して session_id を渡す（フロントでverify→解除）
